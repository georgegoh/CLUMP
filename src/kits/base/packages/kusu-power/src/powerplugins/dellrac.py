...
# -*- coding: utf-8 -*-
#
# $Id: dellrac.py 3126 2009-10-20 07:29:26Z abuck $
#
...
class _RACInstance:
    ...
    def _login(self):
        ...
        self.log.debug("Opening https connection to '%s' (Challenge)", self.host)
        ...
        if RC_handler.get_rc()['code'] != str('0x0'):
            raise Exception, "Failed with error code %s, %s" % \
                  (RC_handler.get_rc()['code'], RC_handler.get_rc()['msg'])

        xml.sax.parseString(xml_content, XML_handler)
        challenge = XML_handler.get_challenge()

        self.log.debug("%s: Challenge '%s'" % (self.host, challenge))
        ...
        self.log.debug("%s: SID %s" % (self.host, session_id))
        ...
        self.log.debug("Opening https connection to '%s' (Authenticate)", self.host)
        conn = httplib.HTTPSConnection(self.host)

        conn.putrequest('GET', '/cgi/login?user=%s&hash=%s' % (self.user, hash))
    ...
    def _executeCommand(self, xml_command):
        ...
        self.log.debug("Opening https connection to '%s' (Command)", self.host)
        conn = httplib.HTTPSConnection(self.host)
        ...
        http_response = conn.getresponse()
        if http_response.status != 200:
            raise Exception, "%s %s" % (http_response.status, str(http_response.reason))
    ...
    def _getFirmwareVersion(self):
        ...
        self.log.debug("RAC Firmware version: %s" % fwrev)
        return fwrev
...
class dellrac(kusu.power.Plugin):
    """
    ...
    def powerOff(self, node, options):
        ...
        if status == kusu.power.STATUSOFF:
            self.log.debug("%s: Node %s is already turned off" % (self.__name__, node))
            return kusu.power.SUCCESS
    ...
    def powerCycle(self, node, options):
        ...
        if status == kusu.power.STATUSON:
            ...
            self.log.debug("%s: powerstatus is OFF - turning on node: %s" % \
                           (self.__name__, node))
            rac.powerOn()
        else:
...
kusu.power.registerPlugin(dellrac)
