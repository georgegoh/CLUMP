...
#
"""The classes in this module model the relationship between a physical disk
and its partitions."""
import sys
import math
import parted
...
from disk_geom import free_space_comparator, getAlignedSectors
...
class Disk(object):
    ...
    __getattr_dict = { 'length' : 'self.pedDevice.length',
                       ...
                       'sectors_per_cylinder' : 'self.pedDisk.dev.heads * self.pedDisk.dev.sectors',
    ...
    def createPartition(self, size, fs_type=None, mountpoint=None, fill=False):
        ...
        #TODO: It's not good to mix the mechanism(create a partition) and policy(boot must be primary) here,
        #remember to refactor policy out when rules grow beyond 1
        #Special rules: if the partition's mountpoint is "boot", then it needs a primary partition
        if (mountpoint != None and mountpoint.lower() == '/boot' and next_partition_type != 'PRIMARY'):
            msg = "Can't create a primary partition of size (%s) bytes for the [boot] mountpoint." % (size)
        ...
        try:
            ...
            raise PartitionException, str(e)
...
#            logger.debug('After delete, key=%d, part.num=%d' % (key, self.partition_dict[key].num))
    ...
    def getAlignedSize(self, start_sector, end_sector):
        start, end = getAlignedSectors(start_sector, end_sector,
                                       self.sectors_per_cylinder)
    ...
    def getLargestSpaceAvailable(self):
        ...
        free_spaces = getSpacesAvailable(self)
        free_spaces.sort(free_space_comparator)
        largest_space_start, largest_space_end = free_spaces[-1]
        largest_space_start = getCylinderBoundariesForSector(largest_space_start, self.sectors_per_cylinder)[0]
        largest_space_end = getCylinderBoundariesForSector(largest_space_end, self.sectors_per_cylinder)[1]
        last_allocatable_sector = (self.cylinders * self.sectors_per_cylinder) - 1

        # don't end on the last sector.
        if largest_space_end > last_allocatable_sector:
            largest_space_end = last_allocatable_sector

        if largest_space_start < largest_space_end:
            largest_size = (largest_space_end - largest_space_start) * self.sector_size
        else:
            largest_size = 0
        return largest_size
    ...
    def __getStartEndSectors(self, size, fill=False):
        ...
        logger.debug('start sector: %d end: %d' % (start_sector, end_sector))
        end_sector = self.__alignEndSector(end_sector)
        logger.debug('Aligned start sector: %d end: %d' % (start_sector, end_sector))
...
#                logger.debug('sector is in use... using new start %d' % start_sector)
        ...
        if end_sector >= self.length or end_sector < start_sector:
            adjusted_size = end_sector - start_sector * self.sector_size
            avail_space = (self.length - start_sector) * self.sector_size
            msg = "Requested partition size required alignment to sector boundaries.\n"
            msg += "This size is too large to fit into the remaining space available "
            msg += "on the disk."
    ...
    def convertEndSectorToCylinder(self, end_sector):
        ...
        if cylinder > self.cylinders:
            logger.debug('Converted cylinder[%d] for end sector[%d] is out of disk range' % (cylinder, end_sector))
    ...
    def commit(self):
        if not self.leave_unchanged:
            ...
            logger.info('Leave Unchanged Flag set for %s' % self.path)
...
class Partition(object):
    ...
    def __init__(self, disk, pedPartition, mountpoint=None):
        ...
        self._label = None
        if self.fs_type in ['ext2', 'ext3', 'linux-swap']:
            l = subprocess.Popen(['/sbin/blkid', self.path],
                                 stdout=subprocess.PIPE,
                                 stderr=subprocess.PIPE)
            out, err = l.communicate()
            if not err:
                try:
                    self._label = out.split('LABEL=')[1].split()[0].strip().strip('"')
                except:
                    self._label = ''
        if self.fs_type in ['reiserfs']:
            cmd = "reiserfstune %s | grep -o 'LABEL: '"
            l = subprocess.Popen(cmd % self.path, shell=True,
                                 stdout=subprocess.PIPE,
                                 stderr=subprocess.PIPE)
            out, err = l.communicate()
            self._label = out.strip('LABEL: ')
    ...
    def setLabel(self, name=''):
        ...
        if self.fs_type not in ['ext2', 'ext3', 'reiserfs', 'linux-swap']:
        ...
        if self.fs_type == 'linux-swap':
            if name:
                self._label = 'KUSU_SWAP'
            else:
                self._label = 'KUSU_SWAP-' + basename(self.getpath())
                logger.debug('set swap label to default value %s' % (self.label))
            return

        if not name:
            name = self.mountpoint
        if self.fs_type == 'reiserfs':
            cmd = 'reiserfstune -l %s %s' % (name, self.path)
        else:
            cmd = '/usr/sbin/e2label %s %s' % (self.path, name)
        logger.debug('Set label cmd: %s' % cmd)
        label = subprocess.Popen(cmd, shell=True,
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE)
        out, err = label.communicate()
        self._label = name
        logger.debug('label out: %s\n\n label err: %s' % (out, err))
    ...
    def format(self):
        ...
        if self.fs_type == 'ext2':
            ...
            logger.info('FORMAT %s: Making swap fs on %s' % \
                        (self.path, self.path))
            mkfs = subprocess.Popen('mkswap -L %s %s' % ('KUSU_SWAP', self.path),  #sf
                                    shell=True,
                                    stdout=subprocess.PIPE,
                                    stderr=subprocess.PIPE)
            mkfs_out, status = mkfs.communicate()
        ...
        elif self.fs_type == 'reiserfs':
            logger.info('FORMAT %s: Making reiser fs on %s' % \
                        (self.path, self.path))
            mkfs = subprocess.Popen('mkreiserfs -f %s' % self.path,
                                    shell=True,
                                    stdout=subprocess.PIPE,
                                    stderr=subprocess.PIPE)
            mkfs_out, status = mkfs.communicate()
