#!/usr/bin/env python
# $Id: lvm.py 3373 2010-01-15 09:10:35Z binxu $
#
...
class LogicalVolumeGroup(object):
    ...
    def __init__(self, name, extent_size='32M', pv_list=[], createNew=False):
        ...
        if (self.extent_size/1024/1024) not in range(2, 512+1) or \
           (self.extent_size/1024/1024) % 2:
        ...
        if extents < self.extents:
            ...
            if extra_extents > self.group.extentsFree():
                raise InsufficientFreeSpaceInVolumeGroupError, \
                      "Cannot extend logical volume because there are not " + \
                      "enough free extents available on the Volume Group."
            else:
