#! /usr/bin/env python
#
# $Id: rpmtool.py 3135 2009-10-23 05:42:58Z ltsai $
#
...
class RPM:
    ...
    def __init__(self, r=None, **kwargs):
            ...
            if type(r) in [str, unicode]:
                ...
                if self.ext  == '.rpm':
                    ...
                    try:
                        ...
                        os.close(fd)
                        raise InvalidRPMHeaderException, r
                    os.close(fd)
                elif self.ext == '.hdr':
                    self._read_header(r)
        ...
        if not self.hdr:
            raise InvalidRPMHeaderException, "Invalid header"
    ...
    def getPost(self):
        ...
        return '#!%s\n%s' % (self.hdr[rpm.RPMTAG_POSTINPROG],self.hdr[rpm.RPMTAG_POSTIN] or '')

    def getPostUn(self):
        """Returns the postun section of the RPM, including the hash bang"""
        return '#!%s\n%s' % (self.hdr[rpm.RPMTAG_POSTUNPROG],self.hdr[rpm.RPMTAG_POSTUN] or '')

    def getPre(self):
        """Returns the pre section of the RPM, including the hash bang"""
        return '#!%s\n%s' % (self.hdr[rpm.RPMTAG_PREINPROG],self.hdr[rpm.RPMTAG_PREIN] or '')
    ...
    def __cmp__(self, other):
        ...
        if n1 == n2:
            ...
            raise RPMComparisonException, "Different rpms"
    ...
    for dir in dirs:
        ...
        for r in dir.files():
            if r.ext == '.rpm' and r.basename() != 'TRANS.TBL':
                try:
                    r = RPM(str(r))
                except InvalidRPMHeaderException, e:
                    if ignoreErrors:
                        continue
                    else:     
                        raise e
