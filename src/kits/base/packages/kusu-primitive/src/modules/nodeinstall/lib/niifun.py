#!/usr/bin/python
#
# $Id: niifun.py 3157 2009-11-02 05:48:56Z yqang $
#
...
#for imaged installs
    ...
    def endElement(self, name):
        if name == 'component':
            ...
            self.imageinfo_start = 0
            self.imageinfo += (self.data)
        self.data = ''


    def characters (self, ch):
        if self.compstart == 1 or self.packstart == 1 or\
                self.scrptstart == 1 or self.debugstart == 1 or\
                self.cfmstart == 1:
            self.data += ch


    def saveAppGlobalsEnv (self, filename=''):
        """saveAppGlobalsEnv - Save the appglobals variable as a shell
...
class NIIFun:
    ...
    def setBootFrom(self, bootfrom):
        """setBootFrom - Set the value of the bootfrom.
        This needs to be used in conjunction with the callNodeboot to
        actually cause the value to be set."""
        self.nextboot = bootfrom

        
    def setState(self, state):
        """setState - Set the value of the state.
        This needs to be used in conjunction with the callNodeboot
        to actually cause the state to be set."""
...
#             self.cfmflag = 0
    ...
    def wantNII(self, flag):
        """wantNII - Set the flag to request the NII data.
        This needs to be used in conjunction with the callNodeboot
