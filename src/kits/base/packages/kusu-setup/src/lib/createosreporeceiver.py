...
class CreateOSRepoReceiver:
    ...
    def create_new_os_repo(self, os):
        ...
        if out:
            print out[0:out.find('You can')-1]
        if err:
            print err[0]
    ...
    def add_kits_to_repo(self, kits_list, repo_name):
        for kit in kits_list:
            ...
            out, err = runCommand(cmd)
            if out:
                print out[0:out.find('Remember')-1]
            if err:
                print err[0]
    ...
    def refresh_repo(self, repo_name):
        ...
        if err:
            print err[0]
            print "The repository was not properly refreshed. Please do a manual refresh once the installer exits."
            message.warning()
        else:
            message.success()
