...
class NetEditApp(object, KusuApp):
    ...
    def run(self):
        ...
        if self.islock():
           self.logErrorEvent(self._("netedit_already_inuse"))
           sys.exit(-1)
        ...
        ks = USXNavigator(screenFactory=screenFactory, screenTitle="Network Edit - Version 0.10", showTrail=False)
        ks.run()
        self.unlock()
...
class NetworkMainWindow(USXBaseScreen):
    ...
    def newAction(self, data=None):
        ...
        ks = USXNavigator(ScreenFactory,screenTitle="Network Edit - Version 0.10", showTrail=False)
        ks.run()
        return NAV_NOTHING    
