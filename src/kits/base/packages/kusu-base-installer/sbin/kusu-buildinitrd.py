#!/usr/bin/python
#
# $Id: kusu-buildinitrd.py 3135 2009-10-23 05:42:58Z ltsai $
#
...
class BuildInitrd:
    ...
    def makeInitrd(self, nodegroup, image, type):
        ...
        # Skip kusu-buildinitrd for sles/opensuse
        ...
        if system_arch == 'i386' and _os.arch == 'x86_64':
            self.stdoutout('Skipping kusu-buildinitrd for %s. Unable to build on %s platform.\n' % (_os.arch, system_arch))
            sys.exit(0)
...
class BuildInitrdApp(KusuApp):
    ...
    def toolVersion(self):
        ...
        global version
        self.stdoutMessage("kusu-buildinitrd version %s\n", self.version)
        sys.exit(0)
