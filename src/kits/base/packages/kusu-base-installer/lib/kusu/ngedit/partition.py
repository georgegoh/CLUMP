...
def getPartList(part_rules, diskprofile):
    ...
    for p in part_rules:
        ...
        if dev.lower() == 'n':
            ...
            # can be an LV or an LVG also , check for that
            if not translatePartitionOptions(p['options'],'vg')[0]\
                    and not translatePartitionOptions(p['options'],'lv')[0]:
                mountpoint = translateMntPnt(p['mntpnt'])
                if mountpoint not in\
                        diskprofile.mountpoint_dict.keys():
                    part_list.append(p)
                    logger.debug('Append to part_list: %s' % p)
    return part_list
