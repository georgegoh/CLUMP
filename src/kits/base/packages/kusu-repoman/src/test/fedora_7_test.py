#!/usr/bin/env python
# $Id: fedora_7_test.py 590 2008-02-01 03:51:13Z ggoh $
#
...
class TestFedora7Repo:
    ...
    def setUp(self):
        ...
        osKit.arch = 'i386'
        osKit.isOS = True
        osComp = db.Components(cname='fedora-7-i386')
        ...
        dirs.append(prefix / 'depot' / 'contrib' / 'fedora' /  '7' / 'i386')
        dirs.append(prefix / 'opt' / 'kusu' / 'lib' / 'nodeinstaller' / 'fedora' / '7' / 'i386')
       
        for dir in dirs:
            dir.makedirs()

        (prefix / 'depot' / 'kits' / 'base' /  '0.1' / 'noarch' / 'kitinfo').touch()
        (prefix / 'opt' / 'kusu' / 'lib' / 'nodeinstaller' / 'fedora' / '7' / 'i386' / 'updates.img').touch()

        for p in self.getPath():
            new_path = prefix / 'depot' / 'kits' / 'fedora' / '7' / 'i386' / p
            try:
        ...
        download('kernel-2.6.9-22.0.1.EL.i386.rpm', \
                 prefix / 'depot' / 'kits' / 'fedora' / '7' / 'i386' / 'Fedora' / 'kernel-2.6.9-22.0.1.EL.i386.rpm')
        ...
        download('ftp-0.17-33.fc6.i386.rpm', \
                 prefix / 'depot' / 'contrib' / 'fedora' / '7' / 'i386' / 'ftp-0.17-33.fc6.i386.rpm')

        download('yum-updatesd-3.0-6.noarch.rpm', \
                 prefix / 'depot' / 'contrib' / 'fedora' / '7' / 'i386' / 'yum-updatesd-3.0-6.noarch.rpm')
    ...
    def testRelativeLinks(self):
        ...
        r = repo.Fedora7Repo('i386', prefix, self.dbs)
        r.test = True
    ...
    def testMake(self):
        ...
        assert (prefix / 'depot' / 'repos' / repoid / 'Fedora' / 'kernel-2.6.9-11.EL.i386.rpm').exists()
        assert not (prefix / 'depot' / 'repos' / repoid / 'Fedora' / 'kernel-2.6.9-22.0.1.EL.i386.rpm').exists()
        assert (prefix / 'depot' / 'repos' / repoid / 'Fedora' / 'ftp-0.17-33.fc6.i386.rpm').exists()
        assert (prefix / 'depot' / 'repos' / repoid / 'Fedora' / 'yum-updatesd-3.0-6.noarch.rpm').exists()
        assert not (prefix / 'depot' / 'repos' / repoid / 'Fedora' / 'yum-updatesd-3.2.2-1.fc7.noarch.rpm').exists()
    ...
    def testGetUpdates(self):
        ...
        rpmsPath = prefix / 'yumupdates' / 'fedora' / 'releases' / '7' / 'Fedora' / 'i386' / 'os' / 'Fedora' 
        newPath = prefix / 'depot' / 'kits' / 'fedora' / '7' / 'i386' / 'Fedora' 
        ...
        updatesDir = prefix / 'depot' / 'updates' / 'fedora' / '7' / 'i386'

        assert len(rpmpkgs) == 3
        # yum-updatesd is newer
        assert (updatesDir / 'yum-updatesd-3.2.2-1.fc7.noarch.rpm').exists() 
        # ftp rpm no change
        assert not (updatesDir / 'ftp-0.17-41.fc7.i386.rpm').exists() 
        # new docbook-utils-pdf rpm in updates
        assert (updatesDir / 'docbook-utils-pdf-0.6.14-8.fc7.noarch.rpm').exists() 
        # newer kernel
    ...
    def testGetPackageFilePath(self):
        ...
        assert p == prefix / 'depot' / 'repos' / str(r.repoid) / 'Fedora' / 'kernel-2.6.9-11.EL.i386.rpm'
