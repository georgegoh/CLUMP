...
class AddHostPlugin(AddHostPluginBase):
      ...
      def finished(self, nodeList, prePopulateMode):
          ...
          if removedMode:
             ...
             wptr.close()
             #use shutil.move which handles moving files across filesystems by
             #falling back to copy if rename fails."
             shutil.move("/tmp/known_hosts.root", "/root/.ssh/known_hosts")
