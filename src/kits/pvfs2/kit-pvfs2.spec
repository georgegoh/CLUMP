...
%define COMP1 component-pvfs2-v2_7
%define kitname pvfs2
...
KID=`/opt/kusu/sbin/sqlrunner -q "SELECT kid FROM kits WHERE rname='pvfs2' and version='%{version}'"`
if [ $? -ne 0 ]; then
   exit 0
fi
...
rm -rf /opt/pvfs2
#rm -rf /opt/kusu/lib/plugins/ngedit/*pvfs2*.py?

# Remove CFM symlinks
for i in `/opt/kusu/sbin/sqlrunner -q 'SELECT ngname FROM nodegroups WHERE ngid = 1 AND ngid < 5'`; do
    if [ -d /etc/cfm/$i/opt/pvfs2 ]; then
       rm -rf /etc/cfm/$i/opt/pvfs2
    fi
done

for i in `/opt/kusu/sbin/sqlrunner -q 'SELECT ngid FROM nodegroups WHERE ngid = 1 AND ngid < 5'`; do
    if [ -d /opt/kusu/cfm/$i/opt/pvfs2 ]; then
       rm -rf /opt/kusu/cfm/$i/opt/pvfs2
    fi
done

## Remove user/group on removal of kit
#if [ `grep -c pvfs2 /etc/passwd ` -eq 1 ]; then
#   /usr/sbin/userdel -r pvfs2
#fi
