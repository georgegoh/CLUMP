...
FILE(GLOB_RECURSE SUPP_DIST_VER_ARCH "${CMAKE_CURRENT_BINARY_DIR}/boot.msg")
FOREACH(supp_dist_ver_arch ${SUPP_DIST_VER_ARCH})
  GET_FILENAME_COMPONENT(dist_ver_arch_path ${supp_dist_ver_arch} PATH)
  STRING(REPLACE "${CMAKE_CURRENT_BINARY_DIR}" "" relative_path ${dist_ver_arch_path})
  STRING(REGEX MATCHALL "[^/]+" path_component_list "${relative_path}/")
  LIST(GET path_component_list 2 KUSU_BUILD_DIST)
  LIST(GET path_component_list 3 KUSU_BUILD_DISTVER)
  LIST(GET path_component_list 4 KUSU_BUILD_ARCH)
  CONFIGURE_FILE(${CMAKE_CURRENT_BINARY_DIR}/src/modules/core/src/bin/kusuenv.sh
                 ${dist_ver_arch_path}/kusuenv.sh)
ENDFOREACH(supp_dist_ver_arch)
...
IF(NOT KUSU_BUILD_DIST OR
  ...
  ELSEIF(EXISTS /etc/redhat-release)
	...
	IF(${content} MATCHES "^Scientific Linux")
	  SET(DISTRO scientificlinux)
	  STRING(REGEX MATCH "[0-9]+" DISTRO_RELEASE ${content})
    ELSEIF(${content} MATCHES "^CentOS")
      SET(DISTRO centos)
      STRING(REGEX MATCH "[0-9]+" DISTRO_RELEASE ${content})
    ELSEIF(${content} MATCHES "^Red Hat")
      SET(DISTRO rhel)
      STRING(REGEX MATCH "[0-9]+" DISTRO_RELEASE ${content})
    ENDIF(${content} MATCHES "^Scientific Linux")
